...
// Generated by metaharness — the `agentic-reports-agent` CLI entry point.
//
// This is plain ESM JavaScript on purpose: it runs as-is via `npx agentic-reports-agent`
...
// "bin" + "files" fields in package.json), so `npx agentic-reports-agent` works the moment
...
const HARNESS_NAME = 'agentic-reports-agent';

/** `agentic-reports-agent init` — boot the kernel + host adapter and report status. */
async function init() {
...
/** `agentic-reports-agent doctor` — verify the install end-to-end (kernel + host resolve). */
